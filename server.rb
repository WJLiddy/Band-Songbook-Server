...
loop do
  Thread.start(server.accept) do |socket|
    ...
    # Await the first JSON packet to tell what room they want to join, or if they want to create.
    ...
    if (request["request"] == "create group")
      bandleader_loop(user,socket)
    end
    if (request["request"] == "join group")
      group_member_loop(user,socket)
    end
  end
end